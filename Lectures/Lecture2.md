# Lecture 2 
 ...
 - Motivating Example 1 (Compounding Interest Rate):
    - A nonlinear function P(t) = $P_0$ * e^Bt^ is commonly used to model a continuous interest process of an investment over time t, where P~0~ denotes the initial investment and B is a given interest rate
 ...
 - Motivating Example 2 (Curvature Fitting):
    ...
    - Since s(t) = $B_1$t +  $B_2$log(t), we can write this as a least squares problem:
        - \[\min_{B_1,B_2} \; \frac{1}{2}\sum_{i=1}^{n}\left(B_1 t_i + B_2 \log(t_i) - \hat{s}(t_i)\right)^2\]